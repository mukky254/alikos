...
router.get('/', async (req, res) => {
  ...
  const alternatives = req.query.alternatives === 'true' ? 'true' : 'false';
  ...
  if (heading !== null && heading >= 0 && heading <= 360) routeOptions.set('bearings', `${Math.round(heading)},45;0,180`);
  const path = `/route/v1/${profile}/${fromLng},${fromLat};${toLng},${toLat}?${routeOptions.toString()}`;
  try {
