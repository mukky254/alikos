...
function bizRow(b) {
  return `<div class="admin-row" data-id="${b.id}">
    <input type="checkbox" class="bulk-check" data-id="${b.id}" style="margin-right:4px;">
...
// Feature: bulk actions. Selects multiple pending listings and approves
// them in one click — loops the same single-item /verify endpoint the
// individual "Verify" button already uses, so no backend change needed.
function wireBulkBar(container, refresh) {
  const bar = document.createElement('div');
  bar.className = 'bulk-action-bar';
  bar.innerHTML = `<button class="btn small" id="bulkVerifyBtn">✓ Verify selected</button><button class="btn small ghost" id="bulkClearBtn">Clear selection</button><span class="muted" id="bulkCount" style="font-size:12px;"></span>`;
  container.parentNode.insertBefore(bar, container);
  const updateCount = () => {
    const n = container.querySelectorAll('.bulk-check:checked').length;
    document.getElementById('bulkCount').textContent = n ? n + ' selected' : '';
  };
  container.addEventListener('change', (e) => { if (e.target.classList.contains('bulk-check')) updateCount(); });
  bar.querySelector('#bulkClearBtn').addEventListener('click', () => { container.querySelectorAll('.bulk-check').forEach((c) => (c.checked = false)); updateCount(); });
  bar.querySelector('#bulkVerifyBtn').addEventListener('click', async () => {
    const ids = [...container.querySelectorAll('.bulk-check:checked')].map((c) => c.dataset.id);
    if (!ids.length) { toast('Select at least one listing first.'); return; }
    if (!confirm(`Verify ${ids.length} business(es)?`)) return;
    for (const id of ids) { try { await api(`/admin/businesses/${id}/verify`, { method: 'POST' }); } catch (e) {} }
    toast(`${ids.length} business(es) verified.`);
    loadStats(); refresh();
  });
}
...
async function loadPending() {
  ...
  const oldBar = el.parentNode.querySelector('.bulk-action-bar');
  if (oldBar) oldBar.remove();
  try {
    const { businesses } = await api('/admin/businesses?status=pending');
    el.innerHTML = businesses.length ? businesses.map(bizRow).join('') : '<p class="muted" style="margin:0;">All caught up.</p>';
    wireBizActions(el);
    if (businesses.length) wireBulkBar(el, loadPending);
